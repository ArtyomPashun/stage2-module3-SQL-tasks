...
SELECT * FROM marks WHERE subject_id IN (SELECT id FROM subject WHERE name LIKE 'Art');

SELECT * FROM student JOIN payment p ON p.student_id = student.id JOIN paymenttype pt ON pt.id = p.type_id WHERE p.name LIKE 'WEEKLY';
